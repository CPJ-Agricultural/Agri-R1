...
# coding: utf-8
"""
Knowledge Inference Script - For agricultural disease knowledge QA tasks (Zero-shot + COT version)
Features:
...
def process_single_sample(entry, processor, model, device, max_new_tokens, max_image_size=896):
    """Process single sample (Zero-shot + CoT mode)"""
    ...
    try:
        ...
        # Build messages (Zero-shot + CoT - no examples)
        messages = [
...
def collate_fn_safe(batch, processor, device, max_size=896):
    """Safer batch collate function (Zero-shot + CoT version)"""
    images = []
    texts = []
    metadata = []
...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=1, help='Batch size (Zero-shot+CoT recommended 1-2)')
    parser.add_argument('--max_new_tokens', type=int, default=2048, help='Maximum generation tokens (CoT needs longer)')
    parser.add_argument('--max_image_size', type=int, default=896, help='Maximum image size')
    args = parser.parse_args()

    print("="*80)
    print("Knowledge Zero-shot + CoT Optimized Inference Script")
    print("="*80)
    ...
    print(f"Strategy: Optimized Zero-shot + CoT + Try batch first, retry single sample on failure")
    print(f"Batch Size: {args.batch_size}")
