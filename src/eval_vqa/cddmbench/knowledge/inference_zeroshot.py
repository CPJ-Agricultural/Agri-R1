...
# ========== System Prompt for Knowledge Tasks ==========
...
# Zero-shot mode uses System Prompt directly without few-shot examples
...
def process_single_sample(entry, processor, model, device, max_new_tokens, use_placeholder_images=False):
    """Process single sample (Knowledge Zero-shot mode)"""
    ...
    try:
        ...
        # Build messages (Knowledge Zero-shot) - Only System Prompt, no examples
        messages = [
            {"role": "system", "content": SYSTEM_PROMPT}
        ]

        # Add current question (Zero-shot: direct question, no examples)
        messages.append({
            "role": "user",
            "content": [
...
def collate_fn_safe(batch, processor, device, max_size=896):
    """Safer batch collate function (Knowledge Zero-shot version)"""
    images = []
    texts = []
    metadata = []
    ...
    for entry in batch:
        ...
        try:
            ...
            print(f"Warning: Sample processing failed {image_path}: {e}")
            continue
...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=4, help='Batch size (recommended 2-4, knowledge QA answers are longer)')
    parser.add_argument('--max_new_tokens', type=int, default=512, help='Maximum generation tokens (knowledge answers are longer)')
    parser.add_argument('--max_image_size', type=int, default=896, help='Maximum image size')
    args = parser.parse_args()

    print("="*80)
    print("Knowledge Zero-shot Inference Script (Agricultural Disease Knowledge QA)")
    print("="*80)
    ...
    print(f"Prompt: Knowledge Zero-shot (no examples) + Optimized Knowledge System Prompt")
    print("="*80)
    ...
    print(f"Strategy: Knowledge Zero-shot (no examples) + Try batch first, retry single sample on failure")
    print(f"Batch Size: {args.batch_size}")
    ...
    for batch_start in tqdm(range(0, total, args.batch_size), desc="Inference progress"):
        ...
        # Print progress every 5 batches (knowledge answers are longer, batch processing is slower)
        if batch_count % 5 == 0:
