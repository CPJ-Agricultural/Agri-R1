...
# coding: utf-8
"""
Optimized Inference Script - Enforced Format Output
Optimization Goals:
1. Ensure all outputs conform to <think>...</think><answer>...</answer> format
2. Improve generation quality and completeness
3. Add output validation and repair mechanisms
"""
import os
import json
import torch
...
# ========== System Prompt (Aligned with zero-shot, with think tags) ==========
...
# Note: This script uses zero-shot prompting - examples have been removed
...
def validate_and_fix_format(text):
    ...
    Ensure output conforms to <think>...</think><answer>...</answer> format
    """
    ...
    has_think = re.search(think_pattern, original_text, re.DOTALL)
    ...
    prefixes_to_remove = [
        'NEW THINK PROCESS:', 'NEWTHINK', 'NEW!', 'NEW THINK:',
        'THINK PROCESS:', 'THINK:', 'ANSWER:', 'THINK ANSWER:'
    ]
    for prefix in prefixes_to_remove:
        if fixed_text.startswith(prefix):
            fixed_text = fixed_text[len(prefix):].strip()

    # Try to identify thinking part (content containing Steps)
    step_pattern = r'(Step 1:.*?)(?=(?:<answer>|$))'
    step_match = re.search(step_pattern, fixed_text, re.DOTALL | re.IGNORECASE)

    think_content = ""
    answer_content = ""

    if step_match:
        think_content = step_match.group(1).strip()
        # Remove possible <think> or other tags
        think_content = re.sub(r'</?think>', '', think_content, flags=re.IGNORECASE).strip()

    # Try to extract answer part
    if has_answer:
        answer_match = re.search(answer_pattern, fixed_text, re.DOTALL)
        if answer_match:
            answer_content = answer_match.group(1).strip()
    else:
        # Try to find content after thinking as answer
        # Look for "THINK ANSWER:" or "ANSWER:" followed by content
        answer_markers = ['THINK ANSWER:', 'ANSWER:']
        for marker in answer_markers:
            if marker in fixed_text:
                parts = fixed_text.split(marker, 1)
                if len(parts) > 1:
                    answer_content = parts[1].strip()
                    break

        # If still not found, try to extract all content after Step
        if not answer_content and think_content:
            remaining = fixed_text.replace(think_content, '', 1).strip()
            # Remove possible markers
            for marker in answer_markers + ['<think>', '</think>']:
                remaining = remaining.replace(marker, '')
            if remaining:
                answer_content = remaining.strip()

    # If no valid think or answer found, return original text with tags
    if not think_content:
        think_content = "Unable to parse structured reasoning from model output."
    if not answer_content:
        # If no answer, use entire text as answer
        answer_content = original_text[:200] if len(original_text) > 200 else original_text

    # Build correct format
    formatted_output = f"<think>{think_content}</think><answer>{answer_content}</answer>"
...
def collate_fn(batch, processor, device):
    ...
    for entry in batch:
        ...
        try:
            ...
            # Build messages (Zero-shot - no examples)
            messages = [
...
def generate_batch(model, processor, inputs, device, batch_size, max_new_tokens=512):
    """
    Batch generation
    ...
    try:
        with torch.no_grad():
            generated_ids = model.generate(
                **inputs,
                max_new_tokens=max_new_tokens,  # Increased to 512 to ensure complete output
                min_new_tokens=50,  # Set minimum length to avoid early stopping
        ...
        for i, response in enumerate(responses):
            ...
            # Log if fixed version differs from original (only when debugging)
            if response != fixed_response and False:  # Set to True to enable debugging
                print(f"\n[Format Fixed] Sample {i+1}:")
        ...
        error_msg = f"<think>Batch generation failed: {str(e)}</think><answer>Generation error occurred.</answer>"
...
def validate_output_quality(results):
    ...
    for result in results:
        ...
        has_steps = 'Step 1:' in solution and 'Step 2:' in solution

        if has_think:
            stats['with_think_tag'] += 1
        if has_answer:
            stats['with_answer_tag'] += 1
        if has_think and has_answer:
            stats['with_both_tags'] += 1
        if has_steps:
            stats['with_steps'] += 1
        if has_think and has_answer and has_steps:
...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=32, help='Batch size (recommended 16-32)')
    parser.add_argument('--num_workers', type=int, default=2, help='DataLoader worker threads')
    parser.add_argument('--max_new_tokens', type=int, default=320, help='Maximum generation tokens')
    args = parser.parse_args()

    print("="*80)
    print("Optimized Inference Script - Enforced Format Output (Zero-shot)")
    print("="*80)
    ...
    # Manual batch processing (avoid DataLoader multiprocessing issues)
    batch_count = 0
    for batch_start in tqdm(range(0, total, args.batch_size), desc="Inference progress"):
        ...
        if inputs is None or valid_count == 0:
            ...
            for entry in batch:
                ...
                new_entry["solution"] = "<think>Failed: Image not found or processing error</think><answer>Unable to process image.</answer>"
        ...
        if batch_count % 10 == 0:
            ...
            if torch.cuda.is_available():
                ...
                print(f"  Format correct: {successful_count}, Format error: {failed_count}")
                print(f"  GPU memory usage: {allocated:.2f}/{total_mem:.2f} GB ({util_pct:.1f}%)\n")
    ...
    print(f"Format error: {failed_count}/{total}")
    ...
    print(f"With <think> tag: {quality_stats['with_think_tag']} ({quality_stats['with_think_tag']/quality_stats['total']*100:.1f}%)")
    print(f"With <answer> tag: {quality_stats['with_answer_tag']} ({quality_stats['with_answer_tag']/quality_stats['total']*100:.1f}%)")
    print(f"With both tags: {quality_stats['with_both_tags']} ({quality_stats['with_both_tags']/quality_stats['total']*100:.1f}%)")
    print(f"With Step format: {quality_stats['with_steps']} ({quality_stats['with_steps']/quality_stats['total']*100:.1f}%)")
    print(f"Complete format (tags+Steps): {quality_stats['complete_format']} ({quality_stats['complete_format']/quality_stats['total']*100:.1f}%)")
    ...
    # Warn if format correctness is below 80%
    ...
    if format_correctness < 80:
        ...
        print(f"\n✓ Format correctness reached {format_correctness:.1f}%")
