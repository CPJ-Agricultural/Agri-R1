...
# coding: utf-8
"""
Five-shot inference script - Uses 5 examples to enhance inference capability
Improvements:
1. Added 5 carefully designed examples
2. Optimized system prompt emphasizing crop and disease identification
3. Complete CUDA error recovery mechanism
...
{"answer": "Your detailed answer including both crop type and disease/pest identification with key symptoms"}
...
EXAMPLES = [
    {
        ...
        "answer": '{"answer": "This is an apple leaf affected by Alternaria Blotch. Key disease symptoms include small circular brown lesions (2-5mm) with distinctive yellowish halos, some coalescing near leaf margins, accompanied by chlorosis indicating moderate disease severity."}'
    },
    {
        "question": "What disease does this tomato leaf have?",
        "answer": '{"answer": "This is a tomato leaf affected by Spider Mites. Key symptoms include fine pale stippling along veins, yellow-bronze roughened appearance, curled leaf edges, dry tissue texture, and faint webbing on the underside indicating active infestation."}'
    },
    {
        "question": "Is this leaf from a cucumber plant?",
        "answer": '{"answer": "No, this is a tomato leaf affected by Late Blight. Key symptoms include irregular water-soaked lesions turning dark brown with pale-green margins, necrotic tissue, and under humid conditions, white downy mold appears on the underside along lesion edges."}'
    },
    {
        "question": "Describe the content of this picture.",
        "answer": '{"answer": "This image shows a tomato leaf affected by Late Blight. Key symptoms include irregular water-soaked lesions turning dark brown with pale-green margins, necrotic tissue, white downy mold on the underside in humid conditions, and potential progression to leaf blighting and petiole collapse."}'
    },
    {
        "question": "Is this apple leaf healthy?",
        "answer": '{"answer": "No, this is not an apple leaf and it is not healthy. This is a grape leaf affected by Leaf Blight, showing symptoms including numerous small dark brown spots with yellow halos, some coalescing into irregular patches, shot-hole tearing, necrotic margins, and general chlorosis between veins."}'
    }
]
...
def check_and_resize_image(image_path, max_size=896):
    """
    Check and resize image dimensions, use smaller size to avoid CUDA issues
    """
    try:
        image = Image.open(image_path).convert("RGB")
        width, height = image.size

        # Use smaller max_size to reduce problems
...
def process_single_sample(entry, processor, model, device, max_new_tokens, use_placeholder_images=False):
    """
    ...
    try:
        ...
        # Add 5 examples (use placeholder images or actual images)
        for i, example in enumerate(EXAMPLES):
            if use_placeholder_images:
                # Use placeholders (pure text mode)
                messages.append({
                    "role": "user",
                    "content": [
                        {"type": "text", "text": f"Example {i+1}: {example['question']}"}
                    ]
                })
            else:
                # Use current image as example image (simplified version)
                messages.append({
                    "role": "user",
                    "content": [
...
def collate_fn_safe(batch, processor, device, max_size=896):
    """
    Safer batch collate function (Five-shot version)
    """
    images = []
    texts = []
    metadata = []
...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=8, help='Batch size (recommended 4-8)')
    parser.add_argument('--max_new_tokens', type=int, default=150, help='Maximum generation tokens')
    parser.add_argument('--max_image_size', type=int, default=896, help='Maximum image size')
    args = parser.parse_args()

    print("="*80)
    print("Five-shot Inference Script (5 examples)")
    print("="*80)
    ...
    print(f"Prompt: Five-shot (5 examples) + Optimized System Prompt")
    print("="*80)
    ...
    # Fix critical issue: set padding_side='left'
    ...
    print(f"Strategy: Five-shot (5 examples) + Try batch first, retry single sample on failure")
    print(f"Batch Size: {args.batch_size}")
