...
# ===== Frequency-Aware (FA) Reward Weighting =====
# Based on Section 5.3: w_freq(c) = 1 + alpha * max(0, theta - f(c))
# alpha=50, theta=0.025 (2.5%)
# Classes with f(c) >= 2.5% keep weight 1.0; rare classes get up to 3.35x amplification
FA_ALPHA = 50.0
FA_THETA = 0.025  # 2.5% threshold

# Training frequency of each crop in CDDMBench (from Table 5 in paper)
CROP_TRAIN_FREQ = {
    "tomato":      0.3719,
    "apple":       0.2948,
    "corn":        0.0835,
    ...
    "raspberry":   0.0161,
    "cherry":      0.0123,
    # crops not listed default to mid-frequency (no amplification needed)
    "peach":       0.0300,
    "strawberry":  0.0200,
    "blueberry":   0.0150,
    "wheat":       0.0300,
...
def get_fa_weight(plant_name: str) -> float:
    """
    Compute frequency-aware reward weight for a given crop.
    w_freq(c) = 1 + alpha * max(0, theta - f(c))
    Returns 1.0 for high-frequency crops, up to ~3.35x for the rarest.
    """
    if plant_name is None:
        return 1.0
    freq = CROP_TRAIN_FREQ.get(plant_name.lower(), FA_THETA)  # default: at threshold
    return 1.0 + FA_ALPHA * max(0.0, FA_THETA - freq)
...
def answer_keyword_reward(completions, solution, **kwargs):
    ...
    Dynamic evaluation:
    - Diagnosis questions (99.6%): plant(0.8) + disease(1.2), scaled by FA weight
    - Treatment questions (0.4%): treatment keywords(2.0)

    FA weighting (Section 5.3): w_freq(c) = 1 + 50 * max(0, 0.025 - f(c))
    Rare crops (<2.5% frequency) receive up to 3.35x reward amplification to
    counteract gradient competition from dominant classes (e.g. Apple at 29.48%).
    The final score is still capped at 2.0 to keep the reward range consistent.
    """
    rewards = []
    ...
    for completion, sol, prob in zip(completions, solution, problems):
        ...
        if prob and is_treatment_question(prob):
            # [Treatment question] Evaluate treatment keywords in Answer (no FA scaling)
            score = evaluate_treatment_in_answer(gen_answer, ref_answer)
        else:
            # [Diagnosis question] Evaluate plant + disease in Answer with FA weighting
            # Default to diagnosis question (99.6% of samples are diagnosis questions)
            base_score, ref_plant = evaluate_diagnosis_in_answer(gen_answer, ref_answer,
                                                                  return_plant=True)
            fa_weight = get_fa_weight(ref_plant)
            score = base_score * fa_weight
...
def evaluate_diagnosis_in_answer(gen_answer, ref_answer, return_plant=False):
    """
    Evaluate diagnosis answer: plant + disease
    Returns: [0, 2.0] score, and optionally the extracted plant name for FA weighting.
    """
    score = 0.0
    ...
    if return_plant:
        return score, ref_plant.lower() if ref_plant else None
    return score
...
def evaluate_logical_coherence(text):
    ...
    causal_patterns = [
        ...
        r'(?:show|display).*?(?:indicate|suggest)',  # 新增
        r'(?:based on|from).*?(?:conclude|determine)',  # 新增
    ]

    for pattern in causal_patterns:
        if re.search(pattern, text_lower, re.DOTALL):
            score += 0.5  # 从0.4提升到0.5
            break
    ...
    if len(steps) >= 2:
        for i in range(1, len(steps)):
            ...
            prev_words = {w for w in prev_words if len(w) > 3}  # 从4降到3
            if prev_words & curr_words:
                score += 0.4  # 从0.3提升到0.4
                break
...
def evaluate_professionalism_context(text):
    ...
    professional_contexts = [
        (r'(?:pathogen|fungus|bacteria|virus)\s+(?:cause|infect|spread|attack)', 0.35),  # 提升
        (r'symptom.*?(?:include|show|indicate|appear|present)', 0.35),  # 提升+扩充
        (r'(?:diagnosis|identify).*?based on', 0.30),
        (r'(?:lesion|spot|blight).*?(?:circular|brown|yellow|dark|light)', 0.25),  # 提升+扩充
        (r'(?:leaf|plant|crop).*?(?:damage|affect|infect|disease)', 0.20),  # 新增
        (r'(?:control|treat|prevent).*?(?:spray|apply|use|fungicide)', 0.20),  # 新增
    ]
...
def evaluate_reasoning_completeness(text):
    ...
    has_observation = any(word in text_lower for word in
                         ['observe', 'see', 'visible', 'appear', 'show', 'display',
                          'symptoms', 'lesions', 'spots', 'brown', 'yellowing',  # 数据集高频
                          'affected', 'leaf', 'leaves'])  # 数据集高频

    # 分析阶段 - 扩充数据集高频词
    has_analysis = any(word in text_lower for word in
                      ['analyze', 'assess', 'examine', 'indicate', 'suggest', 'pattern',
                       'diagnosis', 'infection', 'disease', 'caused',  # 数据集高频
                       'bacterial', 'fungal'])  # 数据集高频

    # 结论阶段 - 扩充数据集高频词
    has_conclusion = any(word in text_lower for word in
                        ['therefore', 'conclude', 'diagnosis', 'identify', 'determine',
                         'control', 'prevent', 'fungicides', 'apply',  # 数据集高频
                         'preventive', 'spray'])  # 数据集高频
