# Stage 1: COT Data Generation Pipeline

This directory contains scripts for generating Chain-of-Thought (COT) training data for agricultural Visual Question Answering (VQA) tasks.

## Overview

The COT generation pipeline consists of 4 main steps:
...
3. **COT Generation** - Generate step-by-step reasoning using external VLM
4. **COT Enhancement** - Evaluate and optimize generated COT quality
...
├── generate_cot.py              # Step 3: COT generation
├── enhance_cot.py               # Step 4: COT enhancement
...
python generate_cot.py \
    ...
    --output train_20k_with_cot.json \
...
- `--target_count` (optional): Total COT samples to generate (default: 200000)
...
1. **Diagnosis Questions** (99.6% of dataset)
   ...
   - COT Structure: Plant ID → Symptom observation → Pattern analysis → Diagnosis

2. **Treatment Questions** (0.4% of dataset)
   - Keywords: "control", "prevent", "treatment", "methods"
   - COT Structure: Disease characteristics → Cultural practices → Chemical control → Application methods
...
## Step 4: COT Enhancement

### Script: `enhance_cot.py`

Evaluates generated COT quality and automatically optimizes low-quality samples.
...
python enhance_cot.py \
    --input train_20k_cot.json \
    --output train_20k_cot_enhanced.json \
...
### Parameters:
- `--input` (required): Input JSON file with COT
- `--output` (required): Output JSON file with enhanced COT
...
Same format as input, with optimized COT for low-quality samples.
...
python generate_cot.py \
    ...
    --output /data/sampled_20k_cot.json \
    --api_key sk-deepseek-xxxxx \
    --api_base https://api.siliconflow.cn/v1 \
    --model_name deepseek-vl2 \
    --target_count 20000

# Step 4: Enhance COT quality
python enhance_cot.py \
    --input /data/sampled_20k_cot.json \
    --output /data/sampled_20k_cot_enhanced.json \
...
- `solution`: Complete solution with COT reasoning
...
Edit the API configuration in `generate_cot.py` and `enhance_cot.py`:

```python
# For COT Generation (generate_cot.py)
API_BASE = "https://api.siliconflow.cn/v1"  # Your VLM API base URL
API_KEY = "sk-xxxxx"                         # Your API key

# For COT Enhancement (enhance_cot.py)
...
- **COT Generation**: DeepSeek-VL2, GPT-4V, Claude-3.5-Sonnet
- **COT Enhancement**: GPT-4o, GPT-4-Turbo, Claude-3-Opus
...
1. **API Rate Limits**
   - Solution: Increase `time.sleep()` delays in generate_cot.py
...
4. **Low COT Quality Scores**
   - Solution: Adjust `--threshold` in enhancement step
   - Or improve few-shot examples in generate_cot.py
...
- **Step 3 (Generate COT)**: ~6-10 hours (API-dependent)
...
### Total Estimated Cost: **$60-120** for 20k high-quality COT samples

---

## Citation

If you use this COT generation pipeline in your research, please cite:
