...
def process_conversation_item(entry, idx, total, total_cot_count):
    ...
    for i, conv in enumerate(conversations):
        if total_cot_count + processed_count >= TOTAL_COT_TARGET:
            print(f"🎯 Reached target of {TOTAL_COT_TARGET} CoT chains, stopping processing")
            break
...
def main():
    ...
    print(f"🎯 Target: Generate {TOTAL_COT_TARGET} CoT chains")
    ...
    for idx, entry in enumerate(data, start=1):
            ...
            for conv in processed_entry["conversations"]:
                if (conv.get("from") == "gpt" and conv.get("think") and
                        not conv["think"].startswith(
                            ("CoT generation failed", "Image read failed", "No corresponding"))):
                    has_valid_cot = True
                    valid_cot_count += 1
        ...
        if total_cot_count > 0 and total_cot_count % SAVE_INTERVAL == 0:
            ...
            print(f"💾 Checkpoint saved: {checkpoint_file} with {total_cot_count} CoT chains")
    ...
    print(f"📈 Average per CoT chain: {avg_time_per_cot:.2f} seconds")

    if total_cot_count >= TOTAL_COT_TARGET:
        print(f"\n🎯 TARGET REACHED: Successfully generated {TOTAL_COT_TARGET} CoT chains!")
