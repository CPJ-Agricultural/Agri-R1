...
2. [COT Generation Data](#cot-generation-data)
...
### Stage 1 Output: Training Data with COT
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `conversations[].think` | string | Yes (for COT) | Chain-of-thought reasoning with 3-4 explicit steps |
...
# - solution: str (reference answer with COT)
```
...
conversations_to_grpo(
    input_file="data/training_with_cot.json",
...
def validate_training_data(data_file, image_dir):
    ...
    for idx, item in enumerate(data):
        ...
        for conv_idx, conv in enumerate(item['conversations']):
            ...
            # Check for COT in assistant messages
            if conv['from'] == 'gpt' and 'think' in conv:
                think = conv['think']
                step_count = len(re.findall(r'Step \d+:', think))
                if step_count < 2:
                    report['errors'].append(f"Sample {idx}, Conv {conv_idx}: COT has only {step_count} steps")
...
report = validate_training_data(
    data_file="data/training_with_cot.json",
...
3. **COT Quality**
   - Ensure 3-4 clear steps per COT
...
1. **File Organization**
   ...
   │   ├── train/
   │   └── test/
   ├── raw/                   # Raw data before COT
   ├── cot_generated/         # After COT generation
   ├── cot_enhanced/          # After quality enhancement
...
**Issue:** COT format inconsistencies

**Solution:**
```python
# Validate and fix COT format
def fix_cot_format(solution):
    import re
...
- [src/stage1_cot/README.md](src/stage1_cot/README.md) - COT generation details
